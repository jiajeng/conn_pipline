function fstep3_create_nproj(procestep,sliceorder,varargin)
    ...
    if isempty(in.sub)
        ...
        subject = in.sub;
        if contains(subject{1},'-')
            sub = cell(size(subject));
    ...
    roifile = cat(2,roifile,roifilepath');
    % -----------------------------------------------------------------------
    ...
    % 2nd level
    Betw_SUB_eff = in.sec_sub_efct;
    Betw_SUB_con = in.sec_sub_con;
    Betw_cond_eff = in.sec_cond_efct;
    Betw_cond_con = in.sec_cond_con;
    ...
    if procesStep.Setup
        ...
        % set 2nd level covariate
        batch.Setup.subjects.effect_names = in.secL_cov_name;
        batch.Setup.subjects.effects = in.secL_cov;
    ...
    if procesStep.snd_Analysis
        ...
        tmp = cellfun(@(x) string(num2str(x)),Betw_cond_con,'UniformOutput',false);
        for neft = 1:length(Betw_cond_con)
            tab = strjoin(string(Betw_cond_eff{neft}),' ');
            projinfo.(['snd_Analysis_condcon_',num2str(neft)]) = cat(1,tab,tmp{neft});
        end

        tmp = cellfun(@(x) string(num2str(x)),Betw_SUB_con,'UniformOutput',false);
        for neft = 1:length(Betw_SUB_con)
            tab = strjoin(string(Betw_SUB_eff{neft}),' ');
            projinfo.(['snd_Analysis_subcon_',num2str(neft)]) = cat(1,tab,tmp{neft});
        end
        ...
        %% CONN 2nd level
        roipath = fullfile(conn_proj_path,conn_proj_name,'results','firstlevel',Analysis_name);
        load(fullfile(roipath,'_list_sources.mat'));
        com_sourcenames = sourcenames(165:186);
        projinfo.roi = com_sourcenames;
        for nroi = 1:length(com_sourcenames)
            for nsueft = 1:length(Betw_SUB_con)
                for nceft = 1:length(Betw_cond_con)
                    batch = [];
                    batch.filename=fullfile(conn_proj_path,[conn_proj_name,'.mat']);
                    batch.Results.analysis_number = Analysis_name;
                    batch.Results.between_subjects.effect_names = Betw_SUB_eff{nsueft};
                    batch.Results.between_subjects.contrast = Betw_SUB_con{nsueft};
                    batch.Results.between_conditions.effect_names = Betw_cond_eff{nceft};
                    batch.Results.between_conditions.contrast = Betw_cond_con{nceft};
            ...
            for nsueft = 1:length(Betw_SUB_con)
                for neft = 1:length(Betw_cond_con)
                    conn_batch( 'Results.analysis_number',Analysis_name, ...
                                ...
                                'Results.between_subjects.contrast',Betw_SUB_con{nsueft}, ...
