function o = checkGUI(app,local_flag,o,connF,ISCF)
    ...
    app.projectpathEditField = uieditfield(app.CONNvariablePanel, 'text',"Value",o.conn_prjPath,"FontSize",12*resizeF);
    ...
    if 0
    % Create UITable
    tab = o.conn_2nd_cond_con;
    if size(tab,1) > 1,tab = tab';end
    tab = cat(1,tab,repmat({[]},10,size(tab,2)));
    tab = cell2table(tab);
    app.UITable = uitable(app.CONNvariablePanel,"Data",tab,"FontSize",12*resizeF);
    app.UITable.ColumnName = o.conn_2nd_cond_effect;
    app.UITable.RowName = {};
    app.UITable.Position = [500*ResizeX 15*ResizeY 150*ResizeY 123*ResizeX];
    app.UITable.Visible = false;
    app.UITable.ColumnEditable = true;

    tab = o.conn_2nd_sub_con;
    if size(tab,1) > 1,tab = tab';end
    tab = cat(1,tab,repmat({[]},10,size(tab,2)));
    tab = cell2table(tab);
    app.UITable2 = uitable(app.CONNvariablePanel,"Data",tab,"FontSize",12*resizeF);
    app.UITable2.ColumnName = o.conn_2nd_sub_effect;
    app.UITable2.RowName = {};
    app.UITable2.Position = [300*ResizeX 15*ResizeY 150*ResizeY 123*ResizeX];
    ...
    app.newprojectCheckBox.Position = [200*ResizeX 74*ResizeY 83*ResizeX 22*ResizeY];
    ...
    % Create DoneButton
    app.DoneButton = uibutton(app.preprocessfiledefinePanel, 'push','ButtonPushedFcn',@(scr,event) DonebutomPush(scr,event,app,local_flag,o),"FontSize",12*resizeF);
...
function DonebutomPush(~,~,app,local_flag,o)
    Ef = 0;
    % define folder nest
    ...
    % tmp = table2array(app.UITable.Data);
    % tmp(cellfun(@isempty,tmp)) = [];
    % app.UIFigure.UserData.conn_2nd_cond_con = tmp;
    % app.UIFigure.UserData.conn_2nd_cond_effect = app.UITable.ColumnName;

    app.UIFigure.UserData.conn_2nd_cond_con = o.conn_2nd_cond_con;
    app.UIFigure.UserData.conn_2nd_cond_effect = o.conn_2nd_cond_effect;
    

    % tmp = table2array(app.UITable2.Data);
    % tmp(cellfun(@isempty,tmp)) = [];
    % app.UIFigure.UserData.conn_2nd_sub_con = tmp;
    % app.UIFigure.UserData.conn_2nd_sub_effect = app.UITable2.ColumnName;

    app.UIFigure.UserData.conn_2nd_sub_con = o.conn_2nd_sub_con;
    app.UIFigure.UserData.conn_2nd_sub_effect = o.conn_2nd_sub_effect;
    
    app.UIFigure.UserData.cov_name = o.cov_name;
    app.UIFigure.UserData.cov_data = o.cov_data;
...
function Tree2checkchange(src,~,app)
    ...
    if any(contains(steps,"Add ROI")) || any(contains(steps,"Setup"))
        app.userdefineROIpathEditFieldLabel.Visible = true;
        app.userdefineROIpathEditField.Visible = true;
    else
        app.userdefineROIpathEditFieldLabel.Visible = false;
